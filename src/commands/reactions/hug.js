    ...
    async execute(interaction) {
        ...
        try {
            if (!fs.existsSync(gifsPath)) {
                return interaction.reply({
                    content: '❌ Папка с GIF для /hug не найдена.',
                    ephemeral: true
                });
            }
            ...
            if (gifs.length === 0) {
                return interaction.reply({
                    content: '❌ В папке с GIF для /hug нет файлов.',
                    ephemeral: true
                });
            }
            ...
            const embed = new EmbedBuilder()
                .setColor(0xff9f43)
                .setDescription(
                    `🤗 ${interaction.user} обнял(а) ${target}!`
                )
